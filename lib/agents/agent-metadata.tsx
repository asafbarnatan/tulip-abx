...
export interface AgentMeta {
  ...
  // Where this agent's output renders in the UI — shown on the agent card so an
  // AE knows where to look for the result, not just what the agent does.
  outputAppears: string
  // 'pipeline' = runs as part of the per-account Full Pipeline (in pipelineStep order).
  // 'portfolio' = runs as a separate cross-account sweep (Run Signal Watch button).
  pipelineRole: 'pipeline' | 'portfolio'
  pipelineStep?: number     // 1..5 for pipeline agents, undefined for portfolio
}

// AGENTS array order = canonical order shown across the UI. Pipeline agents
// come first in dependency-correct sequence (step 1 → 5); the portfolio agent
// (SignalWatcher) is last so the per-account flow reads top-to-bottom cleanly.
export const AGENTS: AgentMeta[] = [
  ...
  },
  {
    ...
    outputAppears: 'Buying Group tab',
    pipelineRole: 'pipeline',
    pipelineStep: 2,
  },
  {
    ...
    outputAppears: 'Positioning Brief tab',
    pipelineRole: 'pipeline',
    pipelineStep: 3,
  },
  {
    ...
    outputAppears: 'Recommended Plays tab + Actions tab',
    pipelineRole: 'pipeline',
    pipelineStep: 4,
  },
  {
    ...
    outputAppears: 'Campaigns tab + Mission Control LinkedIn panel',
    pipelineRole: 'pipeline',
    pipelineStep: 5,
  },
  {
    ...
    outputAppears: 'Mission Control Account Pulse + intent scores',
    pipelineRole: 'portfolio',
  },
]
