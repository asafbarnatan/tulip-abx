...
const { data: acct } = await sb.from('accounts').select('*').ilike('name', '%boston scientific%').maybeSingle()
if (!acct) { console.error('no Boston Scientific account'); process.exit(1) }
...
for (const c of contacts ?? []) {
  console.log(`  ${(c.name || '').padEnd(28)} ${(c.title || '').slice(0, 60).padEnd(60)} role=${c.buying_group_role ?? 'null'}`)
}
...
for (const c of campaigns ?? []) {
  console.log(`  ${c.campaign_name}  [${c.status}]  linkedin_id=${c.linkedin_campaign_id ?? '-'}`)
  console.log(`    impr=${c.impressions}  clicks=${c.clicks}  leads=${c.leads}  $${c.cost_usd}`)
  if (c.headline) console.log(`    headline: ${c.headline}`)
  if (c.ad_copy) console.log(`    ad_copy: ${c.ad_copy}`)
}

console.log('\n=== SIGNALS ===')
const { data: signals } = await sb.from('signals').select('*').eq('account_id', acct.id).order('created_at', { ascending: false })
for (const s of signals ?? []) {
  console.log(`  [${s.signal_type}] source=${s.source ?? '-'}  sentiment=${s.sentiment ?? '-'}  processed=${s.processed}`)
  if (s.content) console.log(`    ${s.content.slice(0, 400)}${s.content.length > 400 ? '…' : ''}`)
  if (s.source_url) console.log(`    url: ${s.source_url}`)
}
