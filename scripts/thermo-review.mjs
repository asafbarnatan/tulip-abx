...
const { data: acct } = await sb.from('accounts').select('*').ilike('name', '%thermo fisher%').maybeSingle()
if (!acct) { console.error('no Thermo Fisher account'); process.exit(1) }
...
for (const c of contacts ?? []) {
  console.log(`  ${(c.name || '').padEnd(28)} ${(c.title || '').slice(0, 70).padEnd(70)} role=${c.buying_group_role ?? 'null'}`)
}
